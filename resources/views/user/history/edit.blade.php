...
@section('content')
    <form novalidate action="{{ route('system.histories.update', ['history' => $history->id]) }}" method="POST" class="form">
        @csrf
        @method('PUT')
                ...
                <select class="@error('category_id') is-invalid @enderror" id="category_id"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}"
                                @if(old('category_id', $history->category_id) == $category->id) selected @endif>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-12 col-lg-8 col-xl-6 mb-3">
                ...
                <select class="@error('type_id') is-invalid @enderror" id="type_id" name="type_id">
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type->id }}" @if(old('type_id', $history->type_id) == $type->id) selected @endif>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-12 col-lg-8 col-xl-6 mb-3">
                ...
                <input type="number" class="@error('amount') is-invalid @enderror" id="amount"
                       name="amount" value="{{ old('amount', $history->amount) }}">
                @error('amount')
                <div class="text-error">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row">
            <div class="col-12 col-lg-8 col-xl-6 mb-3">
                ...
                <textarea rows="10" class="@error('note') is-invalid @enderror" id="note"
                          name="note">{{ old('note', $history->note) }}</textarea>
                @error('note')
                <div class="text-error">{{ $message }}</div>
                @enderror
            </div>
